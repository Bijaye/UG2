...
class MH_Typechecker {
	...
	// ADD CODE HERE
    	if(exp.isBOOLEAN()) return BoolType; //exp.isBOOLEAN()
    	else if(exp.isNUM()) return IntegerType;
    	else if(exp.isVAR()) return env.typeOf(exp.value());
    	else if(exp.isINFIX()){
    		MH_TYPE exp1=computeType(exp.first(),env);
    		MH_TYPE exp2=computeType(exp.second(),env);
    		if(!exp1.isInteger()||!exp2.isInteger()) throw new TypeError("Can't apply "+exp.infixOp()+" on non-Integer types");
    		else{
    			String infix=exp.infixOp();
    			if(infix.equals(("=="))) return BoolType;
    		...
    		MH_TYPE exp2=computeType(exp.second(),env);
    		if(exp1.isArrow()){
    			MH_TYPE left=exp1.left();
    			if(left.equals(exp2)) return exp1.right();
    			else throw new TypeError("Wrong function arguments.");
    		}
    		else throw new TypeError("A function expected for first argument");
    	}
    	else if(exp.isIF()){
    		MH_TYPE exp1=computeType(exp.first(),env);
    		MH_TYPE exp2=computeType(exp.second(),env);
    		MH_TYPE exp3=computeType(exp.third(),env);
    		if(!exp1.isBool()) throw new TypeError("If statement expects a bool type");
    		else if(!exp2.equals(exp3)) throw new TypeError("If statement arguments do not match");
    		else return exp2;
    	}
    	else throw new TypeError("Something went wrong");
    }

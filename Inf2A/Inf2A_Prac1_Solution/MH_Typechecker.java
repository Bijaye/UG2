...
class MH_Typechecker {
    final static MH_Parser MH_Parser = MH_Type_Impl.MH_Parser;

    // Integer type
    final static MH_TYPE IntegerType = MH_Type_Impl.IntegerType;

    // Bool type
    final static MH_TYPE BoolType = MH_Type_Impl.BoolType;
    ...
    throws TypeError, UnknownVariable {
        ...
        if (exp.isVAR()) {
            ...
            // check if payload is a valid integer literal
            try {
                new java.math.BigInteger(exp.value());
                return IntegerType;
            } catch (java.lang.NumberFormatException x) {
                throw new TypeError("Bad integer value " + exp.value());
            }
        }
        else if (exp.isBOOLEAN()) {
            // check if payload is a valid boolean literal
            if (exp.value().equals("True") || exp.value().equals("False")) {
                return BoolType;
            } else {
                throw new TypeError("Bad boolean value " + exp.value());
            }
        }
        else if (exp.isAPP()) {
            MH_TYPE leftType = computeType(exp.first(), env);
            MH_TYPE rightType = computeType(exp.second(), env);
            if (leftType.isArrow() && leftType.left().equals(rightType)) {
                return leftType.right();
            } else {
                throw new TypeError("Type clash in application");
            }
        } 
        else if (exp.isINFIX()) {
            if (computeType(exp.first(), env).equals(IntegerType) &&
                  computeType(exp.second(), env).equals(IntegerType)) {
                String i = exp.infixOp();
                if (i.equals("==") || i.equals("<")) {
                    return BoolType;
                } else if (i.equals("+")) {
                    return IntegerType;
                } else {
                    throw new TypeError("Unknown infix symbol " + i);
                }
            } else {
                throw new TypeError("Infix arguments should be of type Integer");
            }
        }
            ...
            if (computeType(exp.first(), env).equals(IntegerType)) {
                ...
                throw new TypeError("Prefix argument should be of type Integer");
            }
        }
        else if (exp.isIF()) {
            if (computeType(exp.first(), env).equals(BoolType)) {
                MH_TYPE thenType = computeType(exp.second(), env);
                MH_TYPE elseType = computeType(exp.third(), env);
                if (thenType.equals(elseType)) {
                    return thenType;
                } else {
                    throw new TypeError(
                        "Branches of if-expression have different types");
                }
            } else {
                throw new TypeError("if-condition should have type Bool");
            }
        }
        else {
            throw new TypeError("Far-out expression encountered.") ;
        }
    ...
    static class MH_Type_Env implements TYPE_ENV {
         ...
         */
        public MH_TYPE addArgBindings(TREE args, MH_TYPE theType) 

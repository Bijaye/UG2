...
pal_false:                    #//return 0 to caller, not a palindrome
   li $v0,0
   jr $ra  
...
main:
   ...
   la $s4,word                 # char word[20];
                               ...
                               #//use $s3 for char_index, $v0 for delimiting_char
                               # while(1) {
   li $v0,11
   li $a0,'\n'                 # print_char('\n'); 
   syscall     
...
loop_chars:
  ...
  beqz $a0,check_found         # //end of string,  it should work when encounters end of string, even if we keep overwriting

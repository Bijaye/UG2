...
:- multifile at/3, parked/2, delivered/2, connected/2, car/1, agent/1, dirty/2, holding/2, key/2,stored/2.
...
% Impose that the car can only be cleaned while in the parking lot, so that "the agent must clean the parked car
...
% stored state can only be changed by grabbing
stored(K,result(A,S)) :-
  A=store(K);
